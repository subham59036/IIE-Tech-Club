...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  const existing = await dbAdmins.execute({ sql: "SELECT * FROM admins WHERE id = ?", args: [targetId] });
  ...
  if (adminId && adminId !== old.admin_id) {
    ...
    const clash = await dbAdmins.execute({ sql: "SELECT id FROM admins WHERE admin_id = ? AND id != ?", args: [adminId, targetId] });
  ...
  args.push(targetId);
  await dbAdmins.execute({ sql: `UPDATE admins SET ${fields.join(", ")} WHERE id = ?`, args });
...
export async function DELETE(req: NextRequest, { params }: Params) {
  ...
  const existing = await dbAdmins.execute({ sql: "SELECT name, admin_id FROM admins WHERE id = ?", args: [targetId] });
  if (existing.rows.length === 0) {
    return NextResponse.json({ ok: false, error: "Admin not found." }, { status: 404 });
  }
  const target = existing.rows[0] as unknown as { name: string; admin_id: string };

  await dbAdmins.execute({ sql: "DELETE FROM admins WHERE id = ?", args: [targetId] });
